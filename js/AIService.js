...
export class AIService {
    ...
    _manageUnits(aiPlayer) {
        ...
        // 1. Scouts
        const idleScouts = myShips.filter(s => s.type === 'Scout' && !s.targetId && !s.scoutMission);
        idleScouts.forEach(scout => this._commandScout(aiPlayer, scout));

        // 2. Salvagers
        const idleSalvagers = myShips.filter(s => s.type === 'Salvager' && !s.targetId);
    ...
    _formFleets(aiPlayer, myShips) {
        const unassignedCombatShips = myShips.filter(s => !s.fleetId && ['Fighter', 'Frigate', 'Destroyer', 'Cruiser', 'TroopTransport'].includes(s.type) && !s.targetId);
    ...
    _commandFleets(aiPlayer) {
        ...
        aiPlayer.fleets.forEach(fleet => {
            ...
            const isIdle = fleetShips.every(s => !s.targetId);
            if (!isIdle) return;
            ...
            // If in neutral system with planets and have transport, stay to capture.
            ...
            if (!currentSystem.owner && hasUnownedPlanets && hasTransport) return;
            ...
            if (target) {
                console.log(`[AIService] Fleet ${fleet.id} moving to ${target.id}`);
