export class RadialMenu {
    ...
    show(items, x, y) {
        ...
        const radius = items.length > 5 ? 90 : 75; // Use a larger radius for more items
    ...
    hide() {
        if (this.container) {
            const menu = this.container.querySelector('.radial-menu');
            if (menu) {
    ...
    _injectCSS() {
        ...
        const css = `
            #radial-menu-container { position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: 5000; }
            .radial-menu-backdrop { width: 100%; height: 100%; background: rgba(0,0,0,0.05); backdrop-filter: blur(1px); }
            .radial-menu { position: absolute; transform: translate(-50%, -50%) scale(0.8); width: 1px; height: 1px; opacity: 0; transition: all 0.15s cubic-bezier(0.175, 0.885, 0.32, 1.275); }
            .radial-menu.visible { opacity: 1; transform: translate(-50%, -50%) scale(1); }
            .radial-menu-item { position: absolute; width: 64px; height: 64px; display: flex; align-items: center; justify-content: center; transform-origin: center center; transition: transform 0.1s ease-out; }
